...
COLUMN_MAPPING = {
    ...
    'introduction_text': [
        ...
        '12.个人简介.1',
        '12.个人简介.9',
        '13.从哪里得知网协？网协吸引你的点是什么？你对网协和志愿部门的工作有什么理解？对今后有什么期望？:百团大战____',
        '14.这里是一个树洞！'
    ],
    'accept_adjust': ['10.是否接受调剂'],
    'first_preference': ['9.个人志愿:技术部'],
    'second_preference': ['9.个人志愿:数字媒体中心'],
...
ACCEPT_ADJUST_FALSE_VALUES = ['否', 'no',
                              'false', '0', 'n', '不接受', '不同意', '不服从']
...
def normalize_column_names(df):
    ...
    found_mapping = {}
    columns_to_rename = {}
    multi_column_matches = {}  # 记录多列匹配的字段
    ...
    for target_field, possible_names in COLUMN_MAPPING.items():
        ...
        if matched_columns:
            if len(matched_columns) == 1:
                # 单列匹配，直接重命名
                columns_to_rename[matched_columns[0]] = target_field
                found_mapping[target_field] = matched_columns[0]
            else:
                # 多列匹配，记录所有匹配的列
                multi_column_matches[target_field] = matched_columns
                found_mapping[target_field] = matched_columns

    # 重命名单列匹配的列
    df_renamed = df.rename(columns=columns_to_rename)

    # 为多列匹配的字段创建合并列
    for target_field, matched_columns in multi_column_matches.items():
        # 合并多列内容
        ...
        df_renamed[target_field] = df[matched_columns].apply(
...
def convert_excel_to_json(excel_file_path, sheet_name=0, output_file=None):
    ...
    try:
        ...
        for required_field in REQUIRED_FIELDS:
            # 检查字段是否在DataFrame列中或found_mapping中
        ...
        for index, row in df.iterrows():
            try:
                # 直接使用row数据，因为列已经在normalize_column_names中处理了
                ...
                introduction_text = str(row_data.get(
                    'introduction_text', '')).strip()
                # 替换introduction_text中的换行符为<br/>
                introduction_text = introduction_text.replace(
                    '\\n', '<br/>').replace('\n', '<br/>')

                # 处理接受调剂字段
                accept_adjust = normalize_accept_adjust(
                    row_data.get('accept_adjust'))

                # 处理第一志愿
                first_pref_name = str(row_data.get(
                    'first_preference', '')).strip()
                ...
                if 'second_preference' in row_data and pd.notna(row_data['second_preference']):
                    second_pref_name = str(
                        row_data['second_preference']).strip()
                    second_preference_id = DEPARTMENT_MAPPING.get(
                        second_pref_name)
                    if not second_preference_id:
                        print(
                            f"警告：第 {index + 2} 行，未找到第二志愿部门: {second_pref_name}")
                    ...
                    try:
                        if isinstance(row_data['assigned_datetime'], str):
                            assigned_datetime = datetime.strptime(
                                row_data['assigned_datetime'], '%Y-%m-%d %H:%M')
                            assigned_datetime = assigned_datetime.replace(
                                tzinfo=timezone.utc)
...
def parse_arguments():
    ...
    help_params = {
        ...
        'email_cols': ', '.join(COLUMN_MAPPING['email'][:3]) + '...',
